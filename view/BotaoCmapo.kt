...
class BotaoCampo(private val campo: Campo): JButton() {
    init {
        ...
        addMouseListener(MouseClickListener(campo,{it.abrir()},
            {it.alterarMarcacao()}))
        
        campo.onEvento(this::aplicarEstilo)
    }

    private fun aplicarEstilo(campo: Campo, evento: CampoEvento): Unit {
        when(evento){
    ...
    private fun aplicarEstiloExplodido(): Unit {
        background = COR_BG_EXPLOSAO
        text = "X"
    }

    private fun aplicarEstiloAberto(): Unit {
        background = COR_BG_NORMAL
    ...
    private fun aplicarEstiloMarcado(): Unit {
        background = COR_BG_MARCACAO
        foreground = Color.BLACK
        text = "M"
    }

    private fun aplicarEstiloPadrao(): Unit {
        background = COR_BG_EXPLOSAO
        border = BorderFactory.createBevelBorder(0)
        text = ""
    }
