...
/*
Cria uma classe do tipo enum que possui possíveis acontecimentos
em cada campo do tabuleiro.
 */
...
data class Campo(val linha: Int, val coluna: Int) {
    ...
    val quantidadeDeVizinhosMinados: Int get() = vizinhos.filter { it.minado }.size
     ...
     * * Se o campo estiver minado, dispara um evento de [CampoEvento.EXPLOSAO].
     * Caso contrário, abre o campo e realiza a abertura recursiva dos vizinhos
     * que forem considerados seguros.
     */
    ...
    fun alterarMarcacao() {
        if (fechado) {
            marcado = !marcado
            val evento = if (marcado) CampoEvento.MARCACAO
            else CampoEvento.DESMARCACAO
